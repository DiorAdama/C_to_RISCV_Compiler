...
let rec dump_eexpr = function
  ...
  | Egetfield (ex, field) -> Printf.sprintf "%s.%s" (dump_eexpr ex) field
...
let rec dump_einstr_rec indent oc i =
  ...
  | Isetfield (ex1, field, ex2) -> 
      print_spaces oc indent;
      Format.fprintf oc "%s.%s = %s;\n" (dump_eexpr ex1) field (dump_eexpr ex2)
...
let dump_efun oc funname {funargs; funbody} =
  Format.fprintf oc "%s(%s) {\n%a\n}\n"
    funname
    (String.concat "," (List.map (fun (var, t) -> (string_of_typ t) ^ " " ^ var ) funargs))
    dump_einstr funbody
